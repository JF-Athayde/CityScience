...
def gerar_boletim_html_build(
    ...
    Detect the appropriate language for {city} (Portuguese if in Brazil).
    ...
    <head>
        <meta charset="UTF-8">
        <title>Relatório Técnico GreenBuild - {city}</title>
    ...
    <body>
        <header>
            <h1>Relatório Técnico GreenBuild (LEED)</h1>
            <h2>Localização: {city}</h2>
        </header>
        <main>
            <section>
                <h2>Detalhes do Projeto</h2>
                <p><b>Tipo:</b> {project_type} | <b>Meta LEED:</b> {leed_goal} | <b>Área de Foco:</b> {focus_area}</p>
                <p><i>{project_details}</i></p>
            </section>

            <section>
                <h2>Análise Climática para Design Sustentável</h2>
                <ul class="custom-list">
                    <li>Descreva as condições climáticas médias e tendências observadas.</li>
                    <li>Relacione os efeitos sobre eficiência energética e conforto térmico.</li>
                </ul>
                <canvas id="chart1" width="910" height="300" style="width:910px;height:300px;"></canvas>
            </section>

            <section>
                <h2>Recomendações Específicas LEED</h2>
                <ul class="custom-list">
                    <li><b>Eficiência Energética (EA):</b> Sugira medidas adaptadas ao clima de {city}.</li>
                    <li><b>Eficiência Hídrica (WE):</b> Estratégias para otimizar uso de água.</li>
                    <li><b>Materiais e Recursos (MR):</b> Indique materiais de baixo impacto ambiental.</li>
                    <li><b>Qualidade Ambiental Interna (IEQ):</b> Sugestões para conforto e saúde em áreas internas.</li>
                </ul>
            </section>

            <section>
                <h2>Plano de Ação e Próximos Passos</h2>
                <ul class="custom-list">
                    <li>Liste 3 a 5 ações práticas iniciais.</li>
                </ul>
            </section>
        </main>
            ...
            new Chart(ctx, {{
                ...
                options: {{
                    ...
                    plugins: {{
                        ...
                        title: {{
                            display: true,
                            text: 'Tendência de Temperatura em {city}'
                        }}
                    }},
                    scales: {{
                        y: {{ beginAtZero: false }},
                        x: {{ title: {{ display: true, text: 'Período (dias/horas)' }} }}
                    }}
                }}
            }});
        </script>
    ...
    Rules:
    - Fill in all textual content, replacing placeholder bullet points with actual LEED analysis.
    - Keep all HTML, CSS, and Chart.js inline.
    - The response must be a full HTML page, not Markdown, and ready to open in a browser.
    - Do not return any explanation, only the HTML.
    """
...
gerar_boletim_html_build(
    project_details='Temos uma construção na cidade de fortaleza no bairro Aldeota, gostaria de uma certificação Leed Prata', # O texto principal do prompt
    city='Fortaleza, Brazil', 
    project_type='BDC',
    leed_goal='Silver (Prata)',
    focus_area='Indoor',
